...
const cpfText= newUserForm.elements['cpf'];
const firstNameText= newUserForm.elements['firstName'];
const lastNameText= newUserForm.elements['lastName']; 
const birthdayText= newUserForm.elements['birthday']; 
const genderSelect= newUserForm.elements['gender'];     
const emailText= newUserForm.elements['email'];
const cellPhoneNumberText= newUserForm.elements['cellPhoneNumber'];
const homePhoneNumberText= newUserForm.elements['homePhoneNumber'];
const insuranceCardNumberText= newUserForm.elements['insuranceCardNumber'];
...
newUserForm.addEventListener('submit',async (event) => {
    ...
    try {
        ...
        const addedUser = await newUser({'cpf':cpf,'firstName':firstNameText.value,'lastName':lastNameText.value,'birthday':birthdayText.value,'gender':genderSelect.options[genderSelect.selectedIndex].value,'email':emailText.value,'cellPhoneNumber':cellPhoneNumber,'homePhoneNumber':homePhoneNumber,'insuranceCardNumber':insuranceCardNumberText.value});
        window.alert('The User was added successfully.');// Mostrar un mensaje de éxito o redirigir al usuario aquí si es apropiado.
...
backButton.addEventListener('click',() => window.location.href="/User/SelectInsuranceToAddNewUser/SelectInsuranceToAddNewUser.html");
...
async function newUser(user){
    const apiUrl = `/api/users/${insuranceId}`;
    try {
